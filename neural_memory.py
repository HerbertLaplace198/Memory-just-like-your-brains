...
class HashEncoder:
    ...
    def __init__(self, dimensions: int = VECTOR_DIMS):
...
def load_encoder_config(path: Path) -> TextEncoder:
    ...
    if provider == "hash":
        return HashEncoder(int(config.get("dimensions", VECTOR_DIMS)))
...
class NeuralMemory:
    ...
    def remember(
        ...
        record = {
            ...
            "episode": episode or "",
            "concepts": list(dict.fromkeys(item.strip() for item in topics if item.strip())),
            "procedures": list(
    ...
    def _index_canonical_record(self, record: dict[str, object], text: str) -> None:
        ...
        upper_status = "confirmed" if status == "confirmed" else "proposed"
        current_ids = [neuron_id]
        level_specs: list[tuple[int, list[str], str, str]] = [
            (2, [str(record.get("episode", ""))], "episode", "情景记忆"),
            (3, [str(x) for x in record.get("concepts", [])], "member_of", "语义概念"),
            (4, [str(x) for x in record.get("procedures", [])], "used_in", "程序记忆"),
        ...
        for layer, labels, relation, kind in level_specs:
            ...
            for upper_label in clean:
                ...
                upper_id = row["id"] if row else self._create_neuron(
                    layer,
                    upper_label,
                    f"{kind}：{upper_label}",
                    upper_status,
    ...
    @serialized_write
    def import_mdkb(self, mdkb_bin: Path, workspace: Path, limit: int = 0) -> dict[str, int]:
    ...
    def activate(
        ...
        rows = self.db.execute(
            "SELECT * FROM neurons WHERE status NOT IN ('rejected','stale')"
        ).fetchall()
    ...
    def probe(self, query: str) -> tuple[bool, float, list[ActivatedNeuron]]:
        ...
        peak = l1[0].activation if l1 else 0.0
    ...
    def recall(self, query: str, limit: int = 5) -> list[ActivatedNeuron]:
        return [item for item in self.activate(query) if item.layer == 1][:limit]
        ...
        if row["source"].startswith("mdkb:"):
            ...
            if mdkb_bin and workspace:
                try:
                    ...
                    pointer = (self.root / row["path"]).read_text(encoding="utf-8")
    ...
    def _related_atoms(self, neuron_id: str, max_depth: int = 5) -> list[sqlite3.Row]:
        ...
        for _ in range(max_depth):
            ...
            edges = self.db.execute(
                f"SELECT source_id,target_id FROM synapses WHERE source_id IN ({placeholders})",
                tuple(frontier),
            ).fetchall()
            ...
            for edge in edges:
                ...
                if target in seen:
                    continue
                seen.add(target)
                row = self.db.execute("SELECT layer FROM neurons WHERE id=?", (target,)).fetchone()
                if not row:
                    continue
                if row["layer"] == 1:
                    atom_ids.add(target)
        ...
        return self.db.execute(
            f"""SELECT n.*,e.source FROM neurons n
                LEFT JOIN evidence e ON e.id=n.evidence_id
                WHERE n.id IN ({placeholders}) AND n.status NOT IN ('rejected','archived')
    ...
    def compile_obsidian(self) -> dict[str, object]:
        ...
        for concept in concepts:
            ...
            page = topic_dir / f"{safe_filename(concept['label'])}.md"
            ...
            related = self.db.execute(
                """SELECT n.layer,n.label FROM synapses s JOIN neurons n ON n.id=s.target_id
                   WHERE s.source_id=? AND n.layer>3 ORDER BY n.layer,n.label""",
                (concept["id"],),
            ).fetchall()
            related_lines = "\n".join(
        ...
        maintenance_page.write_text(
            ...
            "# 记忆维护中心\n\n"
            "> 本页只展示候选事项。所有写回核心记忆的动作都必须在命令行明确确认。\n\n"
            "## 人工批注候选\n\n" + proposal_lines + "\n\n"
            "## 系统问题\n\n" + issue_lines + "\n\n"
            "## 待审核关系\n\n" + relation_lines + "\n",
            encoding="utf-8",
        ...
        links = "\n".join(f"- [[主题/{path.stem}]]" for path in generated) or "- 暂无主题页"
        home.write_text(
            "---\nview_type: compiled-memory\ngenerated: true\ndo_not_ingest: true\n---\n\n"
            "# 记忆系统首页\n\n"
            "## 主题导航\n\n"
...
def seed_demo(memory: NeuralMemory) -> None:
    samples = [
        (
            "用户希望记忆系统至少包含三层：先知道有没有相关记忆，再低 Token 快速召回，最后可以查看完整原文。",
            ["AI记忆系统"],
            ["偏好可监督且节省Token的工作流"],
        ),
        (
            "详细记忆正文应保存在 Obsidian 或 Markdown 中，数据库索引只是可重建的派生层。",
            ["AI记忆系统", "知识管理"],
            ["偏好可监督且节省Token的工作流"],
        ),
        (
            "长任务可以把大型工具输出卸载到文件，只在上下文保留带 node_id 的 Mermaid 任务画布。",
            ["AI记忆系统", "Token优化"],
            ["偏好可监督且节省Token的工作流"],
        ),
        (
            "重要或不确定的自动记忆应进入 Inbox，经过用户确认后才能成为长期事实。",
            ["AI记忆系统", "记忆治理"],
            ["偏好可监督且节省Token的工作流"],
        ),
        (
            "查询时采用稀疏激活，只返回三到五张记忆卡片；不足时再沿证据指针下钻。",
            ["AI记忆系统", "Token优化"],
            ["偏好可监督且节省Token的工作流"],
        ),
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            ...
            print(json.dumps(
                memory.import_mdkb(args.mdkb_bin, args.workspace, args.limit),
                ensure_ascii=False,
                indent=2,
            ))
